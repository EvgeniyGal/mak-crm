-- Migration to update payment_type from 'till' to 'card' in teacher_salaries table
...
BEGIN
    ...
    FOR constraint_record IN
        SELECT conname
        FROM pg_constraint
        WHERE conrelid = 'public.teacher_salaries'::regclass
          AND contype = 'c'
          AND (
            pg_get_constraintdef(oid) LIKE '%payment_type%'
            OR conname LIKE '%payment_type%'
            OR conname = 'teacher_salaries_payment_type_check'
          )
    LOOP
        EXECUTE 'ALTER TABLE public.teacher_salaries DROP CONSTRAINT IF EXISTS ' || quote_ident(constraint_record.conname);
    END LOOP;
END $$;

-- Also explicitly try dropping by common constraint names
ALTER TABLE public.teacher_salaries DROP CONSTRAINT IF EXISTS teacher_salaries_payment_type_check;
ALTER TABLE public.teacher_salaries DROP CONSTRAINT IF EXISTS teacher_salaries_payment_type_check1;
...
ADD CONSTRAINT teacher_salaries_payment_type_check 

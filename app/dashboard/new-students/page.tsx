...
export default function NewStudentsPage() {
  ...
  // Column definitions for first table
  const testLessonColumns: ColumnDef<StudentWithTestLesson>[] = useMemo(() => [
    {
    ...
    {
      accessorKey: 'interested_courses',
      header: t('students.interestedClasses'),
      cell: ({ row }) => (
        <div className="text-sm text-gray-500">
          {row.original.interested_courses.length > 0 ? row.original.interested_courses.join(', ') : '-'}
        </div>
      ),
    },
    {
      accessorKey: 'joined_courses',
      header: t('students.enrolledClasses'),
      cell: ({ row }) => (
        <div className="text-sm text-gray-500">
          {row.original.joined_courses.length > 0 ? row.original.joined_courses.join(', ') : '-'}
        </div>
      ),
    },
  ], [t])

  // Column definitions for second table
  const firstTimeColumns: ColumnDef<StudentAttendedFirstTime>[] = useMemo(() => [
    {
      accessorKey: 'student_name',
      header: t('dashboard.student'),
      cell: ({ row }) => (
  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <h1 className="text-xl md:text-3xl font-bold text-gray-900 truncate min-w-0">{t('dashboard.newStudents')}</h1>
      ...
      <div className="bg-white rounded-lg shadow p-4 md:p-6">
        <h2 className="text-lg md:text-xl font-semibold mb-4 text-gray-900">{t('dashboard.studentsWithTestLesson')}</h2>
        {studentsWithTestLesson.length === 0 ? (
          <p className="text-gray-500">{t('dashboard.noStudentsWithTestLesson')}</p>
        ) : (
          <DataTable
            columns={testLessonColumns}
            data={studentsWithTestLesson}
            initialPageSize={10}
      ...
      <div className="bg-white rounded-lg shadow p-4 md:p-6">
        <h2 className="text-lg md:text-xl font-semibold mb-4 text-gray-900">{t('dashboard.studentsAttendedFirstTime')}</h2>
        {studentsAttendedFirstTime.length === 0 ? (
          ...
          <DataTable
            ...
            data={studentsAttendedFirstTime}
            initialPageSize={10}
            stickyFirstColumn={true}
            maxHeight="calc(100vh-300px)"
          />
